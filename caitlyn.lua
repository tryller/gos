...
local config = MenuConfig("Caitlyn", "Caitlyn")
...
config.Combo:Slider("Hp", "Hp to use E", 20, 0, 100, 1)

config:Menu("Harass", "Harass")
config.Harass:Boolean("Q", "Use Q", true)
config.Harass:Boolean("W", "Use W", true)

config:Menu("KS", "Killsteal")
config.KS:Boolean("R", "Ks with R", true)

OnUpdateBuff(function(unit, buff)
if config.Harass.W:Value() then
...
local unit = GetCurrentTarget()
	if ValidTarget(unit, 2000) then
		...
		if config.Combo.E:Value() then
			 local EPred = GetPredictionForPlayer(myHeroPos(),unit,GetMoveSpeed(unit),2000,250,950,80,false,false)
			if CanUseSpell(myHero, _E) == READY and ValidTarget(unit, GetCastRange(myHero,_E)) and (GetCurrentHP(myHero)/GetMaxHP(myHero))*100 < config.Combo.Hp:Value() then
...
function Killsteal()
 for i,enemy in pairs(GetEnemyHeroes()) do
      -- GetPredictionForPlayer(startPosition, targetUnit, targetUnitMoveSpeed, spellTravelSpeed, spellDelay, spellRange, spellWidth, collision, addHitBox)
            if CanUseSpell(myHero, _R) == READY and ValidTarget(enemy,GetCastRange(myHero,_R)) and config.KS.R:Value() and GetCurrentHP(enemy)+GetMagicShield(enemy)+GetDmgShield(enemy) < CalcDamage(myHero, enemy, 0,  (225*GetCastLevel(myHero, _R)+25+2.0*GetBonusDmg(myHero))) then
